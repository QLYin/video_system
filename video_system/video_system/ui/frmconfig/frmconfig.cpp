...
void frmConfig::initNav()
{
    ...
    names << "btnConfigSystem" << "btnConfigCard" << "btnConfigNvr" << "btnConfigIpc" << "btnConfigPoll" << "btnConfigUser" << "btnConfigRecord" << "btnConfigOther";
    texts << "基 本 设 置" << "解码卡管理" << "录像机管理" << "摄像机管理" << "轮 询 配 置" << "用 户 管 理" << "录 像 计 划" << "其 他 设 置";
    icons << 0xf085 << 0xe9e8 << 0xe9e8 << 0xe9f5 << 0xea14 << 0xea59 << 0xe6da << 0xf013;
    ...
    int listsize = btns.size();
    //根据配置启用哪些模块
    btns.at(2)->setVisible(false); //录像机
    btns.at(4)->setVisible(AppConfig::EnableConfigPoll);
    btns.at(5)->setVisible(AppConfig::EnableConfigUser);
    btns.at(6)->setVisible(AppConfig::EnableConfigRecord);
    if (listsize > 7)
    {
        btns.at(7)->setVisible(AppConfig::EnableConfigOther);
    }
}
