...
VideoBox::VideoBox(QObject *parent) : QObject(parent)
{
    ...
    //按照行列数生成的可以自行添加其他布局(前面两个是固定的 2/3 布局)
    //1_2x4表示通道1开始2x4行列布局画面(相当于通道1-8按照2行4列排列)
    //9_2x4表示通道9开始2x4行列布局画面(相当于通道9-16按照2行4列排列)
    types.insert("x", QStringList() << "1_2" << "1_3" << "1_4x1" << "1_2x4" << "9_2x4" << "1_4x2" << "1_5x2" << "1_6x2" << "1_7x2");
...
void VideoBox::appendType(int start, int row, int col)
{
    //追加到x布局后面
    QString type = QString("%1_%2x%3").arg(start).arg(row).arg(col);
...
{
    //约定依次是按照顺序控制启用状态
    int count = enable.count();
    if (count > 0 && enable.at(0)) {
        addMenu(menu, "x");
    }
    if (count > 1 && enable.at(1)) {
        addMenu(menu, "4");
    }
    if (count > 2 && enable.at(2)) {
        addMenu(menu, "6");
    }
    if (count > 3 && enable.at(3)) {
        addMenu(menu, "8");
    }
    if (count > 4 && enable.at(4)) {
        addMenu(menu, "9");
    }
    if (count > 5 && enable.at(5)) {
        addMenu(menu, "13");
    }
    if (count > 6 && enable.at(6)) {
        addMenu(menu, "16");
    }
    if (count > 7 && enable.at(7)) {
        addMenu(menu, "25");
    }
    if (count > 8 && enable.at(8)) {
        addMenu(menu, "36");
    }
    if (count > 9 && enable.at(9)) {
        addMenu(menu, "64");
    }
}
