...
DbTableModel::DbTableModel(QObject *parent) : QSqlTableModel(parent)
{
    allCenter = false;
...
bool DbTableModel::eventFilter(QObject *watched, QEvent *event)
{
    //悬停整行换颜色
...
void DbTableModel::setTableView(QTableView *tableView)
{
    //切记这里是 viewport 去识别不然坐标不准
    tableView->viewport()->installEventFilter(this);
}

void DbTableModel::setAllCenter(bool allCenter)
{
    this->allCenter = allCenter;
}

void DbTableModel::setAlignCenterColumn(const QList<int> &alignCenterColumn)
{
    this->alignCenterColumn = alignCenterColumn;
}

void DbTableModel::setAlignRightColumn(const QList<int> &alignRightColumn)
{
